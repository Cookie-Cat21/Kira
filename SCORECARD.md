...
| Deployment | PREVIEW DEPLOYED, LIVE ACCESS BLOCKED | Vercel previews are being created for this branch and are linked from PR #74, but preview and production URLs currently return HTTP 302 to Vercel SSO from `/api/health`, so public judge/live web-quality gates cannot run. Vercel CLI is unauthenticated and opened device login, then was stopped. Need unprotected preview, production promotion, or Vercel auth. |
